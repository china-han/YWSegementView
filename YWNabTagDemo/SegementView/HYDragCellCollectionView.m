...
#pragma mark - UICollectionView (YWDragCellCollectionViewRect)

/**
 内部工具
 */
@interface UICollectionView (YWDragCellCollectionViewRect)
...
/**
 ...
 @return Rect
 */
- (CGRect)YWDragCellCollectionView_rectForSection:(NSInteger)section;
...
/**
 ...
 @return Rect
 */
- (CGRect)YWDragCellCollectionView_rectForRowAtIndexPath:(NSIndexPath *)indexPath;

@end

@implementation UICollectionView (YWRect)
...
- (CGRect)YWDragCellCollectionView_rectForSection:(NSInteger)section {
    ...
    if (sectionNum <= 0) {
        ...
        CGRect firstRect = [self YWDragCellCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        CGRect lastRect = [self YWDragCellCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:sectionNum-1 inSection:section]];
...
typedef NS_ENUM(NSUInteger, YWDragCellCollectionViewScrollDirection) {
    YWDragCellCollectionViewScrollDirectionNone = 0,
    YWDragCellCollectionViewScrollDirectionLeft,
    YWDragCellCollectionViewScrollDirectionRight,
    YWDragCellCollectionViewScrollDirectionUp,
    YWDragCellCollectionViewScrollDirectionDown,
};

@interface YWDragCellCollectionView ()
...
@implementation YWDragCellCollectionView
...
// 处理临界点问题
- (YWDragCellCollectionViewScrollDirection)_setScrollDirection {
    ...
    if (self.bounds.size.width + self.contentOffset.x - _snapedView.center.x < _snapedView.bounds.size.width / 2 && self.bounds.size.width + self.contentOffset.x < self.contentSize.width) {
        return YWDragCellCollectionViewScrollDirectionRight;
    }
    if (_snapedView.center.x - self.contentOffset.x < _snapedView.bounds.size.width / 2 && self.contentOffset.x > 0) {
        return YWDragCellCollectionViewScrollDirectionLeft;
    }
    return YWDragCellCollectionViewScrollDirectionNone;
}
...
- (void)_edgeScroll {
    YWDragCellCollectionViewScrollDirection scrollDirection = [self _setScrollDirection];
    switch (scrollDirection) {
        case YWDragCellCollectionViewScrollDirectionLeft:{
            //这里的动画必须设为NO
            ...
            break;
        case YWDragCellCollectionViewScrollDirectionRight:{
            [self setContentOffset:CGPointMake(self.contentOffset.x + 4, self.contentOffset.y) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x + 4, _snapedView.center.y);
            _lastPoint.x += 4;
        }
            break;
        case YWDragCellCollectionViewScrollDirectionUp:{
            ...
            break;
        case YWDragCellCollectionViewScrollDirectionDown:{
            [self setContentOffset:CGPointMake(self.contentOffset.x, self.contentOffset.y + 4) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x, _snapedView.center.y + 4);
            _lastPoint.y += 4;
        }
    ...
    if (scrollDirection == YWDragCellCollectionViewScrollDirectionNone) {
        return;
    }
        ...
        default: {
            ...
            if (self.delegate
                ...
                for (NSInteger i = 0; i < sec; i++) {
                    if (CGRectContainsPoint([self YWDragCellCollectionView_rectForSection:i], point)) {
                        section = i;
                        break;
                    }
                }
